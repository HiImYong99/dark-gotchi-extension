...
try {
    ...
    if (process.platform === 'win32') {
        // Use PowerShell Compress-Archive on Windows (available on modern Windows systems)
        try {
            execSync('powershell -NoProfile -Command "Compress-Archive -Path * -DestinationPath ../dist.zip -Force"', { cwd: DIST_DIR, stdio: 'inherit' });
            console.log('Zip created: dist.zip');
        } catch (e) {
            console.warn('Failed to zip using PowerShell Compress-Archive.');
            console.warn('You can install a zip utility, run this script from WSL/Git Bash, or create the zip manually.');
        }
    } else {
        execSync(`cd ${DIST_DIR} && zip -r ../dist.zip ./*`, { stdio: 'inherit' });
        console.log('Zip created: dist.zip');
    }
} catch (e) {
